...
class MockContext:
    def __init__(self):
        ...
        self.request_context.lifespan_context.query_runners = []
...
def execute_query(ctx, query, db_index):
    """Wrapper for execute_query with test-specific parameter order"""
    try:
        if db_index < 0 or db_index >= len(ctx.request_context.lifespan_context.query_runners):
            return f"Error: Invalid database index {db_index}"
        
        db_config = ctx.request_context.lifespan_context.query_runners[db_index]
...
def test_execute_query():
    ...
    ctx.request_context.lifespan_context.query_runners = [db_config]
    
    # Mock query
    query = "SELECT * FROM users"
    
    # Test with a valid database index
    result = execute_query(ctx, query=query, db_index=0)
...
def test_execute_query_invalid_index():
    ...
    # Create mock context
    ctx = MockContext()
    ctx.request_context.lifespan_context.query_runners = []
    ...
    assert "Error: Invalid database index" in result
...
def test_describe_table():
    ...
    result = describe_table(ctx, table_name="users", db_index=0)
...
def test_get_table_sample():
    ...
    # Test with valid parameters
    result = get_table_sample(ctx, table_name="users", db_index=0, limit=10)
